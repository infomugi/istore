...
class Image extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('product_id, name, image, status', 'required'),
			array('product_id, status', 'numerical', 'integerOnly'=>true),
			array('name', 'length', 'max'=>150),
			array('image', 'length', 'max'=>255),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id_product_image, product_id, name, image, status', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('id_product_image',$this->id_product_image);
